...
Statusline 始终注册 `session_prompt`。Linux/macOS 会包装 `api.ui.Prompt` 并保留原有 `session_prompt_right` 内容，以便测量右侧宽度；Windows 会包装原生 Prompt，并在 Prompt 下方用右对齐的独立行显示 statusline，避开新版 opencode 的右侧 slot 布局问题。两种路径都会动态截断本插件字段，避免换行挤坏 prompt 布局。
